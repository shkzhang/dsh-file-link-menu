...
function toolTargetOf(node) {
  ...
  if (!path.includes("/") || !looksLikePath(path)) return void 0;
...
var MenuBoundary = class extends import_react.Component {
  state = { failed: false };
...
function FileLinkMenuSurface({ sessionId, controller, pickDirectory, previewFile, t }) {
  ...
  const [caps, setCaps] = (0, import_react.useState)(controller.known);
  ...
  const anchorRect = (0, import_react.useCallback)(() => state?.rect ?? null, [state]);
  const fail = (0, import_react.useCallback)((id, error) => {
  ...
  const pathOf = (0, import_react.useCallback)(
    async (target) => {
  ...
  const showAttachment = (0, import_react.useCallback)((path) => {
  ...
  const saveTo = (0, import_react.useCallback)((id, action, fallback) => {
  ...
  const run = (0, import_react.useCallback)((id) => {
    ...
    try {
      ...
      const file = target;
      const withPath = (use) => {
        void pathOf(file).then(use, (error) => {
          fail(id, error);
        });
      };
      if (id === "file.open") {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
    ...
    toast !== null && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(import_dsh_client_ui_primitives.Toast, { text: toast, onDone: () => {
      setToast(null);
    } })
  ] });
}
